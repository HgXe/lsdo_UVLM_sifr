...
from lsdo_uvlm.uvlm_system.solve_circulations.compute_normal_comp import ComputeNormal
...
class MeshPreprocessingComp(Model):
    ...
    def define(self):
        ...
        for i in range(len(surface_names)):
            ...
            bd_vtx_coords[:, num_pts_chord -1, :, :] = def_mesh[:, num_pts_chord -1, :, :] +\
                                                         0.25 * (def_mesh[:, num_pts_chord -1, :, :] -\
                                                                 def_mesh[:, num_pts_chord -2, :, :])
            ...
            # m = ComputeNormal(
            #     vortex_coords_names=bd_vtx_normal_names,
            #     normals_names=bd_vtx_normal_names,
            #     vortex_coords_shapes=surface_shapes,
            # )
            # self.add(m, name='ComputeNormalVecs')  # shape=(2,3,3)
